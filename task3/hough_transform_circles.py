      ...
      for col in range (self.width):
        if self.gradient[row][col] > threshold:
          for r in range (possible_radious):
            ...
            y0 = row - b
            x0 = col - a
            if (self.inside_image_scope(y = y0, x = x0)):
              self.h_space[y0][x0][r] += 1
...
if __name__ == "__main__":
  ...
  threshold = 80
  htc = HoughTransformCircles(gray, min_radius=30, max_radius=100)
